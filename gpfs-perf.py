...
#    'nfs': ['nfs_read', 'nfs_write', 'nfs_read_ops', 'nfs_write_ops', 'nfs_read_lat', 'nfs_write_lat'],
    'nfs': ['nfs_read', 'nfs_write', 'nfs_read_lat', 'nfs_read_ops', 'nfs_write_lat', 'nfs_write_ops', 'nfs_read_queue', 'nfs_write_queue', 'nfs_read_err', 'nfs_write_err', 'nfs_read_req', 'nfs_write_req'],
    'smb': ['op_outbytes', 'op_inbytes', 'op_count', 'op_time'],
...
class rowInfo_nfs():
    def __init__(self, host, metric, path):
        self.host = host
        self.metric = metric
        self.path = path

class rowInfo_smb():
    def __init__(self, host, metric, syscall, op):
        self.host = host
        self.metric = metric
...
class Funcs():
    ...
    def nfs(self, array):
        return rowInfo_nfs(array[0], array[4], array[2])

    def smb(self, array):
        try: 
          return rowInfo_smb(array[0], 'SMBStats', array[3], array[4])
    ...
    def nfs_query(self, rowinfo):
        return "host=%s,operation=%s,path=%s" % (rowinfo.host, rowinfo.metric, rowinfo.path)

    def smb_query(self, rowinfo):
        return "host=%s,operation=%s,syscall=%s" % (rowinfo.host, rowinfo.op, rowinfo.syscall)
...
def zimon_metrics(metric_group, table, query_filter, numbuckets, period, cli, Debug):
    ...
    for key in keys:
        pieces = key.split('|')
#        print(pieces)
        thisParse = parsefunc( pieces )
        if thisParse is None:
          continue
        InfoRows[index] = thisParse
        index += 1
    timestamp = 0
    filename = ""
    queries = []
